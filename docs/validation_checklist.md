...
- [ ] `R79-CAP-01`: CAP 模式可持续循环测量，不是单发后结束。
- [ ] `R79-CAP-02`: 20nF 档测 `203`，主值连续稳定显示，不闪没。
- [ ] `R79-CAP-03`: 2uF 档测 `205`，主值连续稳定显示，不闪没。
- [ ] `R79-CAP-04`: 200uF 档测电解（正接 CAP、负接 GND）可出稳定量级结果。
- [ ] `R79-CAP-05`: 放空判定按 `ADC<=16` 连续 3 次，阈值判定按 `ADC>=2587` 连续 2 次。
- [ ] `R79-CAP-06`: timeout/error 不清空主值，状态按 `OL/ERR` 展示并可自动进入下一轮。
- [ ] `R79-CAP-07`: 切出 CAP 后三路充电脚回 Hi-Z、`DISCH=高`，再切回可继续测量。
- [ ] `R79-CAP-08`: CAP debug 可见状态机状态、ADC、cycles、timeout/empty_timeout 标志。
- [ ] `R79-REG-01`: 连续切换 `RES->DIODE->CAP->FREQ->RES` 无黑屏/卡死，既有功能无回归。

## T-1.7.10-R1（200R 低阻校正 + CONT ON/OFF）
- [ ] `R710-CONT-01`: 短接时主界面阻值接近 `0Ω`，蜂鸣状态 `ON` 且稳定。
- [ ] `R710-CONT-02`: `10Ω` 时主界面阻值接近 `10Ω`，蜂鸣状态 `ON` 且稳定不抖。
- [ ] `R710-CONT-03`: `100Ω` 时主界面阻值接近 `100Ω`，蜂鸣状态 `OFF` 且稳定。
- [ ] `R710-CONT-04`: CONT 主界面不再出现 `BEEP` 文案，改为 `ON/OFF`。
- [ ] `R710-RES-01`: `2k/20k/200k` 档行为无回归。
- [ ] `R710-REG-01`: `RES/VDC/FREQ/DIODE/CAP` 行为无回归，编译通过。
