...
#define VDC_2V_OFFSET_MV 0
#define VDC_20V_OFFSET_MV 0
...
static inline void vdc_set_range(vdc_ctx_t *ctx, vdc_range_t range)
{
    ...
    if (ctx->active) {
        /* Defer apply to next vdc_tick() to avoid direct HAL tick dependency here. */
        ctx->state = VDC_SM_ENTER;
...
static inline app_err_t vdc_tick(vdc_ctx_t *ctx, uint32_t now_ms)
{
    ...
    if (err != ERR_OK) {
        ...
        ctx->result.vdda_mv = VDC_FALLBACK_VDDA_MV;
        ctx->result.status = VDC_STAT_ERR;
        ctx->result.err = err;
