...
static void build_debug_frame(app_ui_frame_t *frame)
{
    ...
    if ((g_app.mode == MODE_RES) && s->valid) {
        ...
        (void)snprintf(frame->line[1], sizeof(frame->line[1]), "MODE_PHYS:%u",
                       (unsigned)mux_get_mode_phys_ch());
        (void)snprintf(frame->line[2], sizeof(frame->line[2]), "VOLT_PHYS:%u",
                       (unsigned)mux_get_volt_phys_ch());
        if (g_app.vdc.valid) {
            (void)snprintf(frame->line[3], sizeof(frame->line[3]), "RAW:%u", (unsigned)g_app.vdc.raw);
            (void)snprintf(frame->line[4], sizeof(frame->line[4]), "SENSE_mV:%lu", (unsigned long)g_app.vdc.mv_sense);
            (void)snprintf(frame->line[5], sizeof(frame->line[5]), "VIN_mV:%lu", (unsigned long)g_app.vdc.vin_mv);
            (void)snprintf(frame->line[6], sizeof(frame->line[6]), "VDDA:%lu", (unsigned long)g_app.vdc.vdda_mv);
        } else {
            (void)snprintf(frame->line[3], sizeof(frame->line[3]), "RAW:----");
            (void)snprintf(frame->line[4], sizeof(frame->line[4]), "SENSE_mV:----");
            (void)snprintf(frame->line[5], sizeof(frame->line[5]), "VIN_mV:----");
